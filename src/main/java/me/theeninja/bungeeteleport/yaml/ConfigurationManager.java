...
/**
 * Class used to create/manage to plugin configurations.
 * 
 * @author TheeNinja
 *
 */
public class ConfigurationManager {
	...
	public Configuration getNewConfig(String filePath, String[] header) {
		...
		Configuration config = new Configuration(this.getConfigContent(filePath), file, this.getCommentsNum(file));
		return config;
	}
	...
	private File getConfigFile(String file) {
		...
		if(file.contains("/")) {
			...
			if(file.startsWith("/")) {
				configFile = new File(BungeeTeleport.getInstance().getDataFolder() + file.replace("/", File.separator));
			} else {
				configFile = new File(BungeeTeleport.getInstance().getDataFolder() + File.separator + file.replace("/", File.separator));
			}

		} else {
			configFile = new File(BungeeTeleport.getInstance().getDataFolder(), file);
		}
	...
	public void prepareFile(String filePath, String resource) {
		...
		try {
			...
			if(resource != null && !resource.isEmpty()) {
				this.copyResource(BungeeTeleport.getInstance().getResource(resource), file);
			}
	...
	/**
	 * Receives the plugin name from the plugin description.
	 * 
	 * @return the plugin name
	 */
	public String getPluginName() {
		return BungeeTeleport.getInstance().getDescription().getName();
	}
	...
	/**
	 * Returns whether the default configuration value 
	 * differs from a user-set configuration value. Used to decide
	 * whether to set value to default value or no.
	 * 
	 * @param value
	 * @param configurationValue
	 * @return whether the default value differs from the current value
	 */
	
	public static boolean isNull(Configuration config, String identifier) {
		return config.getString(identifier) == null;
	}
}
